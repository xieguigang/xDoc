...
_returns: _
#### CliToken
函数为了保持对Linux系统的兼容性会自动替换\为/符号，这个函数则不会执行这个替换
_returns: _
#### Constrain``2
基类集合与继承类的集合约束
_returns: _
#### CopyTo``1
Copy the value in into target variable and then return the target variable.
_returns: _
#### CopyTypeDef``1
...
_returns: _
#### Description
Get the description data from a enum type value, if the target have no attribute data
...
Run the driver in a new thread, NOTE: from this extension function calls, then run thread is already be started, 
 so that no needs of calling the method again.
 (使用线程的方式启动，在函数调用之后，线程是已经启动了的，所以不需要再次调用方法了)
#### Enums``1
Enumerate all of the enum values in the specific type data.(只允许枚举类型，其他的都返回空集合)
_returns: _
#### FillBlank
...
Get a specific item value from the target collction data using its UniqueID property，
 (请注意，请尽量不要使用本方法，因为这个方法的效率有些低，对于获取类型的集合之中的某一个对象，请尽量先转换为字典对象，在使用该字典对象进行查找以提高代码效率，使用本方法的优点是可以选择忽略参数之中的大小写，以及对集合之中的存在相同的Key的这种情况的容忍)
_returns: _
#### GetLength``1
0 for null object
_returns: _
...
_returns: _
#### InsertOrUpdate``1
Insert data or update the exists data in the dictionary, if the target object with 
...
_returns: _
#### InvokeSet``1
Value assignment to the target variable.(将参数里面的值赋值给参数然后返回)
_returns: _
#### InvokeSet``2
Assigning the value to the specific named property to the target object.
 (将参数之中的值赋值给目标对象之中的指定的属性名称的属性，如果发生错误，则原有的对象不会被修改)
_returns: _
#### Is_NA_UHandle
The target parameter value is NaN or not a real number or not?
 (判断目标实数是否为一个无穷数或者非计算的数字，产生的原因主要来自于除0运算结果或者达到了
 的上限或者下限)
_returns: _
#### IsNullOrEmpty
The object its content is nothing?
_returns: _
#### IsNullOrEmpty``1
...
Merge the target array collection into one collection.
 (将目标数组的集合合并为一个数组，这个方法是提供给超大的集合的，即元素的数目非常的多的，即超过了的上限值)
_returns: _
#### MatrixToVector``1
...
_returns: _
#### ModifyValue``1
Modify target object property value using a specific value provider and then return original instance object.
 (修改目标对象的属性之后返回目标对象)
_returns: _
#### Move
变量移动距离然后返回其移动之前的值
_returns: _
#### MoveNext
plus one and then return its previous value. (p++)
_returns: _
#### NormalizeXMLString
对Xml文件之中的特殊字符进行转义处理
_returns: _
...
_returns: _
#### Randomize``1
Return a collection with randomize element position in the original collection.
 (从原有序序列中获取一个随机元素的序列)
_returns: _
#### RegexParseDouble
Parsing a real number from the expression text by using the regex expression .
...
_returns: _
#### Removes``1
Remove all of the element in the from target list
#### RunDriver
...
_returns: _
#### Union
Get a sub set of the string data which is contains in both collection and
_returns: _
#### Wait
假若条件判断不为真的话，函数会一直阻塞线程，直到条件判断为真
#### π
获取一个实数集合中所有元素的积
_returns: _
